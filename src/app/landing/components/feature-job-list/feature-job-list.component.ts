...
export class FeatureJobListComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private route: ActivatedRoute, private anonyService: AnonymousService, private Route: ActivatedRoute, private JobService: JobService, private authService: AuthenticationService) {
    this.Route.data.subscribe(res => {
      ...
      if (data) {
        ...
        if (auth === null || !auth.hasFinishedProfile) {
          ...
          this.page == data.jobs.pager.totalPages ? this.reachedPageEnd == true : '';
          ...
          this.JobService.getApplicantSavedJobs()
            .subscribe(
              resp => {
  ...
  ngOnInit() {
    ...
    this.anonyService.getCompanyByProfileId(this.compId).subscribe(
      data => {
  ...
  loadJobs() {
    ...
    window.onscroll = () => {
      ...
      if (bottomPosition > elementPosition && this.shouldLoad && !this.reachedPageEnd) {
        ...
        this.anonyService.searchAllJobs('', '', this.compId || '', this.page)
          .subscribe(
            data => {
