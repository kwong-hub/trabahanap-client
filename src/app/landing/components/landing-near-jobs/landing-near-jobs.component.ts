...
export class LandingNearJobsComponent implements OnInit {
  ...
  constructor(private anonyService: AnonymousService, private router: Router, 
  ...
  ngOnInit() {
    if(navigator.geolocation) {
      ...
      err => {
        let {latitude, longitude} = {latitude:14.6042, longitude:120.9822};        
        ({latitude: this.latitude, longitude: this.longitude} = {latitude:14.6042, longitude:120.9822});
  ...
  searchJobs() {
    ...
    this.loading = true;
    this.anonyService.searchJobByProximity(this.latitude, this.longitude, radius, key).subscribe(
      data => {
        this.loading = false;
        if(data.success) {
          this.pinMarkers(data.jobs);
