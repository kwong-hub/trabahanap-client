...
export class JobsListComponent implements OnInit {
  ...
  styleObject = { 'inputContainer': {}, 'inputHeader': { backgroundColor: "#fff", fontSize: "1.5rem", border: "1px solid #888" }, 'optionContainer': { backgroundColor: "#555", top: "3.4rem", boxShadow: '0px 1px 2px #aaa' }, 'option': { fontSize: "1.5rem", borderBottom: "1px solid #ddd", backgroundColor: '#fff' } };
  ...
  ngOnInit() {
    ...
    if (auth === null || !auth.hasFinishedProfile) {
      ...
      this.JobsService.getApplicantSavedJobs()
        .subscribe(
          data => {
            // console.log(data)
  ...
  loadJobs() {
    ...
    window.onscroll = () => {
      ...
      if (bottomPosition > elementPosition && this.shouldLoad && !this.reachedPageEnd) {
        ...
        this.anonyService.advancedSearch(val.query || '', this.industryName || '', val.employmentType || '', val.SalaryRange || '', this.cityName || '', val.pwd ? 1 : 0, this.page)
          .subscribe(
            data => {
  ...
  AdvancedSearch() {
    ...
    this.anonyService.advancedSearch(val.query || '', this.industryName || '', val.employmentType || '', val.SalaryRange || '', this.cityName || '', val.pwd ? 1 : 0, 1)
      .subscribe(
        data => {
          // console.log(data);
          this.filterHidden = true;
