...
import { faArrowCircleRight, faEllipsisV, faTimes, faPlus, faTimesCircle, faCheckCircle } from '@fortawesome/free-solid-svg-icons';
...
export class CompanyLocationsComponent implements OnInit {
  ...
  locations = [];
  displayedColumns: string[] = ['picture', 'locationName', 'email', 'headOffice', 'action'];
  company: any;
  public pager: any;
  public page: any;
  companyId;
  ...
  getServerData(page) {
    this.adminService.getCompanyLocation(page.pageIndex + 1, page.pageSize, this.companyId)
      .subscribe(
        success => {
