...
export class AddJobsComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    this.route.params.subscribe(
      params => {
        if (params.jobId) {
          this.jobService.getJobById(params.jobId)
            .subscribe(
              success => {
  ...
  getLocations(){
    this.adminService.getCompanyLocationsByCompanyId(this.companyId)
      .subscribe(
        response => {
          ...
          if (locations) {
            ...
            if (locations) { this.addJob.controls['locationId'].setValue(locations[0].id); }
          }
  ...
  getIndustries() {
    this.otherService.getAllIndustries()
      .subscribe(
        response => {
  ...
  onSubmit() {
    if (!this.descriptionOnly && this.job) { return; }
    this.submitted = true;
    ...
    if (this.job) {
      ...
      this.adminService.editCompanyJob(this.job.id, this.job)
        .subscribe(
          success => {
            //console.log(success)
    ...
    this.adminService.addCompanyJob({ ...val }, this.companyId)
      .subscribe(
        success => {
          if (success.success) {
            this.jobAdded = true;
