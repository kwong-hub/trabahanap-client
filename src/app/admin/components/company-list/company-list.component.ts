...
import { faPlus, faSlidersH, faEllipsisV, faCheck, faTimes, faCheckCircle, faTimesCircle } from '@fortawesome/free-solid-svg-icons';
...
export class CompanyListComponent implements OnInit {
  ...
  verifyCompany(id) {
    this.adminService.verfifyEmployer(id)
      .subscribe(
        data => {
          //console.log(data)
  ...
  countotalJobs() {
    this.companies.forEach(comp => {
      this.adminService.getAllJobs(1, 2, comp.id).subscribe(
        data => {
  ...
  getServerData(page) {
    if (this.filtered) {
      ...
      this.adminService.getFilterEmployers(val.companyName, val.industry, page.pageIndex + 1,page.pageSize)
        .subscribe(
          data => {
            console.log(data);
            if (data) {
      ...
      this.adminService.getAllEmployers(page.pageIndex + 1, page.pageSize)
        .subscribe(
          success => {
  ...
  filterEmployers() {
    ...
    console.log(val);
    this.filterHidden = true;
    this.adminService.getFilterEmployers(val.companyName, val.industry, this.page || 1,8)
      .subscribe(
        data => {
          console.log(data);
          if (data) {
