...
@Component({
  selector: 'employer-mini-job-display',
...
export class MiniJobDisplayComponent implements OnInit {
  ...
  ngOnInit() {
    this.job ? this.job.applicationStartDate = this.job.applicationStartDate.split('T')[0] : null;
    this.job ? this.job.applicationEndDate = this.job.applicationEndDate.split('T')[0] : null;
  }
