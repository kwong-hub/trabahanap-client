...
import { faCheck, faUserPlus, faIdCard, faCloudUploadAlt, faUserCheck, faEyeDropper, faEdit } from '@fortawesome/free-solid-svg-icons';
...
export class AddCompanyProfileComponent implements OnInit {
  ...
  constructor(
    ...
    private locationService: LocationService, private anonyService: AnonymousService
  ) {   }
  ...
  ngOnInit() {
    ...
    document.addEventListener("click", ()=> { this.showIndustries = false });
  } // ngOnInit ends here

  get form(){ return this.addCompanyProfileForm.controls }
  ...
  fetchIndustries(term: string): void {
    if(term === '') { this.industries = []; return; }

    this.industrySearchTerms.next(term);
    this.INDUSTRIES$.subscribe(
      data => {
        // console.log(data)
  ...
  onSubmit() {
    this.submitted = true;
    if(this.addCompanyProfileForm.invalid){return;}
    ...
    this.employerService.addCompanyProfileWithFile(this.formData)
      .subscribe(
        response => {
  ...
  onEdit(){
    ...
    this.employerService.editCompanyProfile({...val, id: this.companyProfile.id, CityId: val.cityId, RegionId: val.regionId, CountryId: val.countryId}, this.companyProfile.id)
      .subscribe(
        response => {
          if(response.success){
