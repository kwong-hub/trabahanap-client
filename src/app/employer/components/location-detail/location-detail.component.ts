...
export class LocationDetailComponent implements OnInit {
    ...
    this.employerService.getCompanyLocationById(this.id).subscribe(
      data => {
        if(data.success) {
          ...
          this.updateInputs();
          ({latitude:this.latitude, longitude:this.longitude} = data.location);
  ...
  onUpdatePicture() {
    this.uploading = true;
    this.employerService.editCompanyBranchPicture(this.imageData, this.id)
      .subscribe(
        data => {
          console.log(data)
          this.formImgSrc = data.location.picture;
  ...
  onSubmit() {
    ...
    this.employerService.editCompanyBranch(newLocation, this.id)
      .subscribe(
        data => {
          console.log(data)
          this.loading = false;
