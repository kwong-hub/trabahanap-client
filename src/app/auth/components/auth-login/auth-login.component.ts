...
export class AuthLoginComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private authenticationService: AuthenticationService, private route: Router, private router: ActivatedRoute, private authService: AuthService) {
  ...
  ngOnInit() {
    ...
    this.authService.authState.subscribe((user) => {
      if(user && user.authToken && this.login){
        if(user.facebook){
          this.authenticationService.facebookLogin(user.authToken, user.id, user, 'applicant')
            .subscribe(
              response => {
                if(response.success){
          ...
          this.authenticationService.googleLogin(user.authToken, user.id, user, 'applicant')
            .subscribe(
              response => {
                if(response.success){
                  this.authenticationService.saveSocialUser(response.user);
    ...
    .subscribe(
      res => {
        ...
        if(res.success && res.user.valid){
          ...
          this.route.navigate(['/auth/set-password'], { queryParams: { token: res.user.token} });
        }else{
  ...
  navigateUser(role){
    ...
    switch(role){
      case Role.applicant: this.route.navigate([this.returnUrl]);
        break;
      case Role.employer: this.route.navigate([this.returnUrl]);
        break;
      case Role.admin: this.route.navigate([this.returnUrl]);
        break;
      case Role.staffer: this.route.navigate([this.returnUrl]);
        break;
      case Role.adminStaff: this.route.navigate([this.returnUrl]);
        break
