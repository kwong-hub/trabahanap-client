...
export class AnonymousService {
  ...
  searchJobByProximity(lat, long, distance, term): Observable<any> {
    return this.http.get(`${environment.apiUrl}/search/location?lat=${lat}&long=${long}&distance=${distance}&key=${term}`)
  }
  searchAllJobs(key,city,companyId,page): Observable<any>{
    return this.http.get<any>(`${environment.apiUrl}/search?key=${key}&cityName=${city}&page=${page}&compId=${companyId}`);
  }
  advancedSearch(key,industry,employtype,salaryRange,cityName,pwd,page){
    return this.http.get<any>(`${environment.apiUrl}/search/advanced?search=${key}&et=${employtype}&industry=${industry}&sr=${salaryRange}&ct=${cityName}&pwd=${pwd}&page=${page}`);
  }
